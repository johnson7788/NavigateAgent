...
def create_app(host: str, port: int, agent_url: str = "") -> Starlette:
    ...
    """
    logger.info("启动 PPT Agent 服务")
    streaming = os.environ.get("STREAMING") == "true"
    logger.info(f"流式模式: {streaming}")

    agent_card_name = "ppt Q&A Agent"
    agent_name = "ppt Q&A Agent"
    # Agent描述必须清晰
    agent_description = "PPT generation service"
...
@click.option("--port", default=10071)
...
app = create_app(host=os.environ.get("HOST", "0.0.0.0"),
                 port=int(os.environ.get("PORT", "10071")),
